...
const SkeletonLoader = () => (
  <div className="w-full space-y-6 animate-pulse px-4 md:px-0 mt-10">
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4"><div className="h-24 bg-white/5 rounded-xl border border-white/5"></div><div className="h-24 bg-white/5 rounded-xl border border-white/5"></div><div className="h-24 bg-white/5 rounded-xl border border-white/5"></div><div className="h-24 bg-white/5 rounded-xl border border-white/5"></div></div>
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-8"><div className="h-96 bg-white/5 rounded-2xl border border-white/5"></div><div className="h-96 bg-white/5 rounded-2xl border border-white/5"></div></div>
  </div>
);
...
function MainApp() {
  ...
  // API Saisons & Périodes
  const [valorantSeasons, setValorantSeasons] = useState([]);
  const [selectedPeriod, setSelectedPeriod] = useState('challenge');
  ...
  useEffect(() => {
    const handleResize = () => {
      if (window.innerWidth < 1280) { setIsMobile(true); setIsSidebarOpen(false); }
      else { setIsMobile(false); setIsSidebarOpen(true); }
    };
    ...
    return (
      <div className="min-h-screen bg-[#0f1923] flex items-center justify-center p-4">
        <div className="text-center space-y-6 bg-[#1c252e] p-10 rounded-2xl border border-white/10 shadow-2xl max-w-lg">
          <div className="mx-auto w-16 h-16 bg-[#ff4655] rounded-xl flex items-center justify-center rotate-3 shadow-[0_0_30px_rgba(255,70,85,0.4)]"><Crosshair size={32} className="text-white" /></div>
          <h1 className="text-3xl font-black text-white italic uppercase tracking-tighter">Installation Réussie !</h1>
          <button onClick={() => navigate('/admin')} className="bg-white/10 hover:bg-white/20 text-white font-bold py-3 px-8 rounded-lg flex items-center gap-2 mx-auto transition-colors"><Settings size={18} /> Panel Admin</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="h-[100dvh] w-full bg-[#0f1923] text-gray-100 font-sans selection:bg-[#ff4655] selection:text-white flex flex-col xl:flex-row overflow-hidden relative">
      <div className="fixed inset-0 z-0 pointer-events-none opacity-20"><div className="absolute top-0 left-0 w-full h-full bg-gradient-to-br from-[#ff4655]/10 via-[#0f1923] to-[#0f1923]"></div><div className="absolute -top-20 -left-20 w-96 h-96 bg-[#ff4655] rounded-full blur-[128px]"></div></div>

      {isMobile && isSidebarOpen && <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-40" onClick={() => setIsSidebarOpen(false)} />}

      <motion.aside
        initial={false} animate={{ x: isMobile ? (isSidebarOpen ? 0 : '-100%') : 0 }} transition={{ duration: 0.3, ease: "easeInOut" }}
      ...
      >
        <div className="p-6 border-b border-white/5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="bg-[#ff4655] p-2 rounded-lg shadow-[0_0_15px_rgba(255,70,85,0.4)] shrink-0"><Crosshair size={24} className="text-white" /></div>
            <h1 className="text-xl font-black tracking-tighter uppercase italic text-white drop-shadow-md whitespace-nowrap">KSL<span className="text-[#ff4655]">TRACKER</span></h1>
          </div>
        ...
        <div className="p-4 border-t border-white/5 bg-black/20 space-y-4">
          <div className="space-y-2">
            <div className="relative">
              ...
              <select value={selectedPlayerId} onChange={handlePlayerChange} className="w-full bg-[#0f1923] text-white text-xs font-bold py-2.5 pl-9 pr-4 rounded-xl border border-white/10 outline-none focus:border-[#ff4655]/50 appearance-none hover:bg-[#1a2733] transition-colors cursor-pointer uppercase">
            ...
            <div className="relative">
              ...
              <select value={selectedPeriod} onChange={(e) => setSelectedPeriod(e.target.value)} className="w-full bg-[#0f1923] text-gray-300 text-xs font-bold py-2.5 pl-9 pr-4 rounded-xl border border-white/10 outline-none focus:border-[#ff4655]/50 appearance-none hover:bg-[#1a2733] transition-colors cursor-pointer uppercase">
                <option value="challenge">DEPUIS LA DATE FIXÉE</option>
                <option value="global">TOUT L'HISTORIQUE</option>
                <optgroup label="Saisons Officielles">
                  {valorantSeasons.map(s => (
                    <option key={s.uuid} value={s.uuid}>{s.displayName}</option>
                  ))}
                </optgroup>
              </select>
            </div>
          ...
          <button onClick={() => forceRefreshFromRiot()} disabled={loading} className="w-full bg-gradient-to-r from-[#ff4655] to-[#d93442] hover:from-[#e03543] hover:to-[#bf2d3a] text-white rounded-xl flex flex-col items-center justify-center gap-1 py-3 text-xs font-black transition-all shadow-lg shadow-[#ff4655]/20 disabled:opacity-80 disabled:cursor-wait">
            {loading ? (
              <><div className="flex items-center gap-2 justify-center"><RefreshCw size={14} className="animate-spin" /> TRAITEMENT...</div>{refreshStatus && <div className="text-[9px] text-white/80 font-bold uppercase tracking-wider text-center px-1">{refreshStatus}</div>}</>
            ) : (<div className="flex items-center gap-2 justify-center"><RefreshCw size={14} /> RAFRAÎCHIR LES STATS</div>)}
          </button>
          ...
          <div className="flex justify-between items-center px-2">
            ...
            <button onClick={() => navigate('/admin')} title="Administration" className="text-gray-500 hover:text-white transition-colors"><Settings size={14} /></button>
          </div>
        </div>
      </motion.aside>
      ...
      {isMobile && (
        <div className="fixed top-4 left-4 z-40">
          <button onClick={() => setIsSidebarOpen(true)} className="bg-[#1c252e]/90 p-2.5 rounded-xl border border-white/10 text-white shadow-[0_0_15px_rgba(0,0,0,0.5)] backdrop-blur-md hover:bg-white/10 transition-colors"><Menu size={24} /></button>
        </div>
      )}
                ...
                {
                  {
                    ...
                    'arsenal': <Arsenal matches={globalFilteredMatches} selectedPlayerId={selectedPlayerId} playersConfig={playersConfig} challengeStartDate={challengeStartDate} />,
                    'maps': <MapAnalysis matches={globalFilteredMatches} playersConfig={playersConfig} challengeStartDate={challengeStartDate} />,
