...
export const AdminPanel = () => {
    ...
    const startEditPlayer = (p) => {
        ...
        setEditPlayerForm({ name: p.name, tag: p.tag, color: p.color, discord_id: p.discord_id || '', crosshair_code: p.crosshair_code || '' });
    };
                                ...
                                <div key={p.id} className="flex items-center justify-between p-4 border-b border-white/5 hover:bg-white/5 transition-colors">
                                    ...
                                    {editingPlayerId === p.id ? (
                                        <div className="flex-grow flex items-center gap-2 mr-4">
                                            ...
                                            <input type="text" placeholder="Code viseur (0;P;...)" value={editPlayerForm.crosshair_code} onChange={e => setEditPlayerForm({ ...editPlayerForm, crosshair_code: e.target.value })} className="bg-[#0f1923] text-white px-2 py-1.5 rounded border border-white/10 text-sm font-mono w-40 outline-none focus:border-blue-500" title="Code de partage du viseur Valorant" />
                                        </div>
                                    ) : (
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-[#1c252e] p-6 rounded-xl border border-white/5 space-y-4">
                            ...
                            <div className="pt-4 border-t border-white/5">
                                <h3 className="font-black text-cyan-400 uppercase mb-1">Maintenance : Réparer l'ADR</h3>
                                <p className="text-[11px] text-gray-500 mb-3">
                                    Recalcule l'ADR (dégâts par round) de tout l'historique depuis les données déjà stockées. Aucun appel API, instantané. Corrige le bug où l'ADR était vide.
                                </p>
                                <button
                                    type="button"
                                    onClick={async () => {
                                        showMsg("Réparation ADR en cours...", "success");
                                        try {
                                            const res = await fetch(`${LOCAL_SERVER_URL}/api/admin/backfill-adr`, {
                                                method: 'POST', headers: authHeaders
                                            });
                                            const json = await res.json();
                                            if (!res.ok) throw new Error(json.error || 'Erreur');
                                            showMsg(`ADR réparé : ${json.updated} matchs corrigés (${json.skipped} ignorés).`);
                                        } catch (e) {
                                            showMsg(`Erreur ADR : ${e.message}`, "error");
                                        }
                                    }}
                                    className="w-full bg-cyan-500/20 hover:bg-cyan-500/40 text-cyan-400 font-black py-3 rounded uppercase tracking-wider transition-colors"
                                >
                                    Réparer l'ADR de l'historique
                                </button>
                            </div>
                        </div>
                    </div>
...
// ==========================================
// (COMPOSANT GESTION DES DONNÉES déplacé dans admin/DataManagement.jsx)
// ==========================================
